# -*- coding: utf-8 -*-
"""bGeigie-Zen Detector Code.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1-Io_M5dtcMPqqv6tqqi5NOqYg87GIX9p
"""

# This is the computer program code, use to analyse the data from the cosmic on air project taken by the bGeigie-Zen Detector
# Valen Lebepe
# 13 September 2025
...
df_d6cm = pd.read_csv("SafeCast 6cm.txt",sep=",", skiprows= 1)    # 6cm distance data
df_d3cm = pd.read_csv("SafeCast 3cm.txt",sep=",", skiprows= 1)    # 3m distance data
df_d1cm = pd.read_csv("SafeCast 1cm.txt",sep=",", skiprows= 1)    # 2m distance data
df_db = pd.read_csv("SafeCast Distance Background.txt",sep=",", skiprows= 1)    # background distance data

df_l = pd.read_csv("SafeCast Lead.txt",sep=",", skiprows= 1)     # lead data
df_lb = pd.read_csv("SafeCast Lead Background.txt",sep=",", skiprows= 1)     # background lead data
df_d1cm.head()

# Cleaning and filtering the data

# Removing un-needed columns
df_d6cm=df_d6cm.drop(["$BNRDD", "5087", "2025-08-14T10:30:09Z","68","19574","A", "0000.0000","N","0000.0000.1", "E","0.0","V","0","0*6C"], axis =1)
df_d3cm=df_d3cm.drop(["$BNRDD", "5087", "2025-08-14T10:45:56Z","44","29675","A", "0000.0000","N","0000.0000.1", "E","0.0","V","0","0*65"], axis =1)
df_d1cm=df_d1cm.drop(["$BNRDD", "5087", "2025-08-14T11:03:57Z","66","43551","A", "0000.0000","N","0000.0000.1", "E","0.0","V","0","0*68"], axis =1)
df_db=df_db.drop(["$BNRDD", "5087", "2025-08-14T11:20:13Z","7","52330","A", "0000.0000","N","0000.0000.1", "E","0.0","V","0","0*60"], axis =1)

df_l=df_l.drop(["$BNRDD", "5087", "2025-08-14T09:00:04Z","10","417","A", "0000.0000","N","0000.0000.1", "E","0.0","V","0","0*6A"], axis =1)
df_lb=df_lb.drop(["$BNRDD", "5087", "2025-08-14T12:30:59Z","1","58658","A", "0000.0000","N","0000.0000.1", "E","0.0","V","0","0*6D"], axis =1)
...
time_d6cm =pd.DataFrame(np.linspace(0,600,len(df_d6cm)), columns=["time"])    # Elapsed time for 6cm distance measurements
...
u_r = 0.1/(2*np.sqrt(3))  # Uncertainty of radius of the detector     - Type b evaluation
r =  4.8/2   # radius of the detector)
u_d = 0.2/(2*np.sqrt(6))  # Uncertainty of distance between source and detector     - Type b evaluation

# For 6cm distance
solid_6cm =  solid_angle_square(r,6) # solid angle at 6cm
u_solid_6cm = solid_angle_uncertainty(r, u_r, 6, u_d)  # uncertainty of solid angle at 6cm
print(solid_6cm,"+/-", u_solid_6cm)

# For 3cm distance
solid_3cm =  solid_angle_square(r,3) # solid angle at 3cm
u_solid_3cm = solid_angle_uncertainty(r, u_r, 3, u_d)  # uncertainty of solid angle at 3cm
print(solid_3cm,"+/-", u_solid_3cm)

# For 1cm distance
solid_1cm = solid_angle_square(r,1)# solid angle at 6cm
u_solid_1cm = solid_angle_uncertainty(r, u_r, 1, u_d)  # uncertainty of solid angle at 1cm
...
plt.title("SafeCast Detector efficiency vs. distance")
plt.grid(True, linestyle ="--" ,alpha=0.4)
plt.legend()
plt.savefig("SafeCast Detector efficiency vs. distancece.png")
plt.show()